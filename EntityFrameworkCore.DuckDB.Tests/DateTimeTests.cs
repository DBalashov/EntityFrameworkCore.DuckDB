...
public class DateTimeTests : BaseTest
{
    ...
    public void UtcNow()
    {
        var query = ctx.Items.Where(p => p.DateTime < DateTime.UtcNow).ToQueryString();

        var offset = -(int) TimeZoneInfo.Local.GetUtcOffset(DateTime.Now).TotalMinutes;
        Assert.True(query.Split(Environment.NewLine).Last()
                         .EndsWith($"< date_add(cast(now() as timestamp), to_minutes({offset}))"));
    }

    [Test]
    public void Now()
    {
        var query = ctx.Items.Where(p => p.DateTime < DateTime.Now).ToQueryString();
        Assert.True(query.Split(Environment.NewLine).Last()
                         .EndsWith("< cast(now() as timestamp)"));
    }
    ...
    public void PropertiesNotNullable()
    {
        ...
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.DayOfYear   > 1).ToQueryString(),                regex, "dayofyear"),    nameof(DateTime.DayOfYear));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.DayOfWeek   > DayOfWeek.Monday).ToQueryString(), regex, "dayofweek"),    nameof(DateTime.DayOfWeek));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Year        > 1).ToQueryString(),                regex, "year"),         nameof(DateTime.Year));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Month       > 1).ToQueryString(),                regex, "month"),        nameof(DateTime.Month));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Day         > 1).ToQueryString(),                regex, "day"),          nameof(DateTime.Day));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Hour        > 1).ToQueryString(),                regex, "hour"),         nameof(DateTime.Hour));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Minute      > 1).ToQueryString(),                regex, "minute"),       nameof(DateTime.Minute));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Second      > 1).ToQueryString(),                regex, "second"),       nameof(DateTime.Second));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Millisecond > 1).ToQueryString(),                regex, "milliseconds"), nameof(DateTime.Millisecond));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTime.Microsecond > 1).ToQueryString(),                regex, "microseconds"), nameof(DateTime.Microsecond));
    }
    ...
    public void PropertiesNullable()
    {
        ...
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.DayOfYear   > 1).ToQueryString(),                regex, "dayofyear"),    nameof(DateTime.DayOfYear));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.DayOfWeek   > DayOfWeek.Monday).ToQueryString(), regex, "dayofweek"),    nameof(DateTime.DayOfWeek));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Year        > 1).ToQueryString(),                regex, "year"),         nameof(DateTime.Year));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Month       > 1).ToQueryString(),                regex, "month"),        nameof(DateTime.Month));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Day         > 1).ToQueryString(),                regex, "day"),          nameof(DateTime.Day));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Hour        > 1).ToQueryString(),                regex, "hour"),         nameof(DateTime.Hour));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Minute      > 1).ToQueryString(),                regex, "minute"),       nameof(DateTime.Minute));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Second      > 1).ToQueryString(),                regex, "second"),       nameof(DateTime.Second));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Millisecond > 1).ToQueryString(),                regex, "milliseconds"), nameof(DateTime.Millisecond));
        Assert.IsTrue(ValidateFirstGroup(ctx.Items.Where(p => p.DateTimeNullable.Value.Microsecond > 1).ToQueryString(),                regex, "microseconds"), nameof(DateTime.Microsecond));
    }
